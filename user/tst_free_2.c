...
void _main(void)
{
	...
	cprintf("3: Free some allocated blocks [Merge with next ONLY]\n\n") ;
	{
		...
		is_correct = 1;
		blockIndex = 0 ;
		cprintf("index of freed = %d\tsize of freed = %d\n", blockIndex, get_block_size(startVAs[blockIndex]));
		...
		struct BlockMetaData* temp = LIST_FIRST(&BlockList);
		cprintf("temp size = %d\ttmp free = %d\n", temp->size, temp->is_free);

		struct BlockMetaData* temp2 = LIST_NEXT(temp);
		cprintf("temp2 size = %d\ttmp2 free = %d\n", temp2->size, temp2->is_free);
