		...
		for (int j = 0; j < allocCntPerSize; ++j)
		{
			...
			if(va == NULL || (va != (curVA + sizeOfMetaData())))
			{
				if (is_correct)
				{
					...
					//cprintf("alloc_block_xx #3.%d: WRONG ALLOC - alloc_block_xx return wrong address. Expected %x, Actual %x\n", idx, curVA+ sizeOfMetaData() ,va- sizeOfMetaData());
					//cprintf("alloc_block_xx #3.%d: WRONG ALLOC - alloc_block_xx curVA %x, sizeOfMetaData %x,"
					//		" va - sizeOfMetaData %x, curVa + sizeOfMetaData %x\n", idx, curVA , sizeOfMetaData(), curVA + sizeOfMetaData());
...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	eval = 0;
//[3.3]increase size + next is empty (call ff)
	cprintf("	3.3: increase size + next is empty (call alloc ff)\n\n");
	{
		is_correct = 1;

		blockIndex = 5 * allocCntPerSize - 1;/*999*/
		old_size = allocSizes[4];/*2k*/
		new_size = old_size + 4 * kilo - sizeOfMetaData();/*6k - meta*/

		void* ptr = startVAs[blockIndex];

		void * expectedAdd = (void*) alloc_block_FF(new_size);
		free_block(expectedAdd);
		va = (void *) realloc_block_FF(ptr, new_size);

		//new blockIndex = 1201;
		if (va == NULL || va != expectedAdd)
		{

			cprintf("test_realloc_block_FF #3.3.1: WRONG REALLOC - it return wrong address. Expected %x, Actual %x\n",expectedAdd, va);
		}


		block_size = get_block_size(expectedAdd);
		//startVAs[1200]
		if (block_size != new_size + sizeOfMetaData())
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.3.2: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",new_size + sizeOfMetaData(), block_size);
		}
		...
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.3.3: WRONG REALLOC! block status (is_free) not equal 0 after realloc.\n");
		}
		//checking next block
		//cprintf("va == startVa[1200]?\n%x == %x\n%x\n%x\n%x\n", va,startVAs[1200], startVAs[1200] - sizeOfMetaData(),startVAs[1201], startVAs[1202]);

		//check content of reallocated block
		if (*(startVAs[1200]) != 1200 || *(midVAs[1200]) != 1200 || *(endVAs[1200]) != 1200)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.3.5: WRONG REALLOC! content of the block is not correct. Expected %d\n",blockIndex);
		}

		//check resized block[999]
		//cprintf("size[999] = %d\tfree[999] = %d\n", get_block_size(startVAs[999]), is_free_block(startVAs[999]));

		//cprintf("%d\n", old_size + (2 * sizeOfMetaData()));
		if(get_block_size(startVAs[999]) != old_size + (2 * sizeOfMetaData()) || is_free_block(startVAs[999]) != 1)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.3.6: WRONG REALLOC! make sure to change the is_free of the resized block.\n");
		}
	...
	//[3.4]increase size take all next
	cprintf("\tTesting inc size take all next\n");
	{
		blockIndex = 2*allocCntPerSize - 1; /*399*/
		old_size = allocSizes[1]; /*20 + meta = 36*/
		new_size = old_size + (1*kilo) - sizeOfMetaData();//400 = 1k -> 0
		va = realloc_block_FF(startVAs[blockIndex], new_size);
		//check reallocated block size & status
		block_size = get_block_size(startVAs[blockIndex]) ;
		if (block_size != new_size + sizeOfMetaData())
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.4.1: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",new_size + sizeOfMetaData(), block_size);
		}
		block_status = is_free_block(startVAs[blockIndex]) ;
		if (block_status != 0)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.4.2: WRONG REALLOC! block status (is_free) not equal 0 after realloc.\n");
		}
		//check vanishing block (if any)
		if (get_block_size(startVAs[blockIndex+1]) != 0 || is_free_block(startVAs[blockIndex+1]) != 0)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.4.3: WRONG REALLOC! make sure to ZEROing the size & is_free values of the vanishing block.\n");
		}
		//check content of reallocated block
		if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex ||	*(endVAs[blockIndex]) != blockIndex)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.4.4: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
		}
		if (is_correct)
		{
			eval += 20;
	...
	//[3.5]testing sbrk
	cprintf("\t[3.5]Testing reallocating block with very large size (calling SBRK)\n");
	{
		blockIndex = 0*allocCntPerSize + 2;
		old_size = allocSizes[0];/*4k*/
		new_size = 3*Mega;

		va = realloc_block_FF(startVAs[blockIndex], new_size);
		if(va != NULL)
		{
			cprintf("tst_realloc_FF #3.5.1: WRONG REALLOC - it returns wrong address.\n");
		}
		//checking if it's still saved
		if(old_size != get_block_size(startVAs[blockIndex]) || is_free_block(startVAs[blockIndex]) != 0)
		{
			cprintf("tst_realloc_FF #3.5.2: WRONG REALLOC - missing block.\n");
		}
		//check content of reallocated block
		if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.5.3: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
		}
	...
	//[3.6]inc size & next is full
	cprintf("\t3.6: increase size but next block is full\n");

	is_correct = 1;
	{
		blockIndex = 300;
		old_size = allocSizes[1];/*36B*/
		new_size = allocSizes[1] + (kilo/2);
		...
		//new blockIndex = 199 -> 4k
		if (va == NULL || va != expectedAdd)
		{

			cprintf("test_realloc_block_FF #3.6.1: WRONG REALLOC - it return wrong address. Expected %x, Actual %x\n",expectedAdd, va);
		}
		...
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.6.2: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",new_size + sizeOfMetaData(), block_size);
		}

		block_status = is_free_block(startVAs[400]);
		if (block_status != 0)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.3.3: WRONG REALLOC! block status (is_free) not equal 0 after realloc.\n");
		...
		//checking vanishing block
		if (get_block_size(va + new_size) != 0 || is_free_block(va + new_size) != 0)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.3.4: WRONG REALLOC! make sure to ZEROing the size & is_free values of the vanishing block.\n");
		}
		//check content of reallocated block
		if (*(startVAs[400]) != 400 || *(midVAs[400]) != 400 || *(endVAs[400]) != 400)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.3.5: WRONG REALLOC! content of the block is not correct. Expected %d\n",blockIndex);
		}

		//check resized block[300]
		//cprintf("size[300] = %d\tfree[300] = %d\n", get_block_size(startVAs[300]), is_free_block(startVAs[300]));

		if(get_block_size(startVAs[300]) != allocSizes[1] || is_free_block(startVAs[300]) != 1)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.3.6: WRONG REALLOC! make sure to change the is_free of the resized block.\n");
		}
	...
	//[4.2]dec size next block is empty
	cprintf("	4.2: decrease size & next block is empty (coalesce)\n\n");

	is_correct = 1;
	{
		blockIndex = 3*allocCntPerSize - 1; /*599*/
		old_size = allocSizes[2] /*1KB*/;
		new_size = old_size - (kilo/4) - sizeOfMetaData();
		...
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #4.2.1: WRONG REALLOC - it return wrong address. Expected %x, Actual %x\n", startVAs[blockIndex] ,va);
		}

		//check reallocated block size & status
		block_size = get_block_size(va);//(startVAs[blockIndex]) ;
		//cprintf("new blk size(with meta) = %d\n", block_size);
		if (block_size != new_size + sizeOfMetaData())
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #4.2.2: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",new_size + sizeOfMetaData(), block_size);
		}
		block_status = is_free_block(startVAs[blockIndex]) ;
		if (block_status != 0)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #4.2.3: WRONG REALLOC! block status (is_free) not equal 0 after realloc.\n");
		}

		//check size and status of next block
		struct BlockMetaData * newblk = (struct BlockMetaData *)(va + new_size);
		expected_size = allocSizes[3] + (kilo/4);
		if (newblk->size != expected_size || newblk->is_free != 1)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #4.2.4: WRONG REALLOC! newly created block is not correct... check it!. Expected %d, Actual %d\n", expected_size, newblk->size);
		}
		//TODO:content
		//check content of reallocated block
		if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex ||	*(endVAs[blockIndex]) != blockIndex)
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #4.2.5: WRONG REALLOC! content of the block is not correct. Expected %d Actual %d\n", blockIndex, *(midVAs[blockIndex]));
		}
