...
/*******************************/
/* PARAM VALIDATION FOR SYSCALLS */
/*******************************/

//self addition function for clean code.
bool param_valid (uint32 va, uint32 s){
	uint32 lastframe = USER_LIMIT - PAGE_SIZE;
	bool va_invalid = (va == 0 || va>= USER_LIMIT);
	bool size_invalid = (va == lastframe && s > PAGE_SIZE);
	uint32 endaddr = va + s;
	bool sizeinrange = (s > 0 && endaddr < USER_LIMIT);
	if (va_invalid || size_invalid || !sizeinrange){
		return 0;
	}
	else {
		return 1;
...
{
	if(!param_valid(virtual_address, size)){
...
static uint32 sys_get_hard_limit(void)
{
	//return hardLimit;
	return curenv->uhardLimit;
}
...
void* sys_sbrk(int increment)
{
	...
	if(increment > 0)
	{
		...
		env->ubrk = old_brk + increment;
		env->ubrk = ROUNDUP(env->ubrk, PAGE_SIZE);
		if(env->ubrk > env->uhardLimit)
		{
			env->ubrk = old_brk;
			return (void *)-1;
		}
		...
		{
			uint32 framesNo = (env->ubrk - old_brk) / PAGE_SIZE;
			uint32 freeFrames = LIST_SIZE(&free_frame_list);
			if(framesNo > freeFrames)
			{
				env->ubrk = old_brk;
				return (void *)-1;
			}
			...
			{
				for(uint32 i = old_brk; i < env->ubrk; i += PAGE_SIZE)
				{
					uint32 *ptr_page_table = NULL;
					int tstate = get_page_table(env->env_page_directory, i, &ptr_page_table);
					if(tstate != TABLE_IN_MEMORY)
					{
						ptr_page_table = create_page_table(env->env_page_directory, i);
					}

					ptr_page_table[PTX(i)] = ptr_page_table[PTX(i)] | PERM_AVAILABLE;
				}
	...
	else if(increment < 0)
	{
		...
		env->ubrk = old_brk + increment;
		uint32 dealloc_start = ROUNDUP(env->ubrk, PAGE_SIZE);

		for(uint32 i = dealloc_start; i < old_brk; i += PAGE_SIZE)
		{
			uint32* ptr_page_table = NULL;

			int tableExists = get_page_table(env->env_page_directory, i, &ptr_page_table);
			if(tableExists == TABLE_IN_MEMORY)
			{
				ptr_page_table[PTX(i)] = ptr_page_table[PTX(i)] & (~PERM_AVAILABLE);

			}

			struct FrameInfo *frame_ptr = get_frame_info(env->env_page_directory, i, &ptr_page_table);
			if(frame_ptr != NULL)
			{
		...
		if(env->ubrk < env->uheapStart)
		{
			env->ubrk = env->uheapStart;
			//return (void *)-1;?
		}
